...
COVERAGE_GAPS = [
    # SQL built in a variable, then executed — taint across statements.
    ("gap-sql-indirect", "python", "CWE-89",
     "def get(cur, uid):\n    q = f\"SELECT * FROM users WHERE id = {uid}\"\n    cur.execute(q)\n",
     "def get(cur, uid):\n    q = \"SELECT * FROM users WHERE id = ?\"\n    cur.execute(q, (uid,))\n"),
    # Insecure randomness for a security token (needs semantic context).
    ("gap-weak-random", "python", "CWE-330",
     "import random\ntoken = str(random.random())\n",
     "import secrets\ntoken = secrets.token_hex(16)\n"),
    # Path traversal — user input flowing into open().
    ("gap-path-traversal", "python", "CWE-22",
     "def read(req):\n    return open(req.args.get('f')).read()\n",
     "import os\ndef read(req):\n    name = os.path.basename(req.args.get('f'))\n    return open(os.path.join(SAFE_DIR, name)).read()\n"),
    # SSRF — user-controlled URL reaching an outbound request.
    ("gap-ssrf", "python", "CWE-918",
     "import requests\ndef fetch(req):\n    return requests.get(req.args.get('url')).text\n",
     "import requests\ndef fetch(req):\n    if not allowed(req.args.get('url')):\n        raise ValueError('blocked')\n    return requests.get(req.args.get('url')).text\n"),
    # XML external entity — parser configured elsewhere.
    ("gap-xxe", "python", "CWE-611",
